...
(function () {
    ...
    var shareScripts = {
        ...
        pins: {
            script: function (callback) {
                $('.share-button-pinterest').each(function () {
                    var el = $(this),
                        newShareURL = '',
                        newHref = wizard + '/share/redirect/',
                        postID = el.data('postId'),
                        postImage = encodeURIComponent(el.data('postImage')),
                        shareURL = el.data('shareUrl'),
                        shareDescription = el.data('shareDescription'),
                        uid = Cookies.get('heleoid'),
                        currentURL = window.location.host + window.location.pathname;
                        console.log(postImage);
                    if (postID && postImage && uid) {
                        newHref += postID + '/' + uid + '/' + postImage;
                        newHref += '?shareref=' + encodeURIComponent(currentURL);
                    }
                    newHref = encodeURIComponent(newHref);
                    newShareURL += 'http://pinterest.com/pin/create/bookmarklet/?media=' + newHref;
                    newShareURL += '&url=' + shareURL + '&is_video=false';
                    newShareURL += '&description=' + shareDescription;
                    console.log(newHref);
                    console.log(newShareURL);
                    el.attr('href', newShareURL);
                });
                callback();
            }
        }
     ...
     */
    $('body').on('mouseover', '.share-button, .share-button-small', function (e) {
        var el = $(e.target),
    ...
    function relateShare(postID, shareType) {
        ...
        if (!sessionShares[postID][shareType]) {
            window.wizardAPI.put('/share/' + postID, data).then(function (resp) {
                currentShareCounter = $('.post-' + postID + ' .share-counter');
...
(function ($) {
    ...
    var modules = {},
        ...
        HeleoTracking = {},
        client = new Keen({
            projectId: "5627b6dd672e6c3771f43f59",
            writeKey: "24147cb172233b9b06d49e30442063fb97c5edfd5554a53b9691d8c9ab8abacf6b55c05f6b49f12107fe97c8148b59ca6146c2b5321cf42d78d738a9a66d5c031971d3aa1f3f2725e73f8d03779352c35e06872625313df88d10295bd0fb6337d30d56ca3105a11cc86664c3d3ef2252"
        });

    HeleoTracking.Event = {
        trackImpression: function (moduleName) {
            if (!moduleName) return;
            client.addEvent("module-impressions", {
                module: moduleName,
                url: window.location.pathname,
                user_agent: navigator.userAgent
            });
        },
        trackPageview: function (pageviewID) {
            if (!pageviewID) pageviewID = '';
    ...
    HeleoTracking.Module = {

        registerModule: function (element, callback, repeat) {
            var id = module_id++;
            modules[id] = {
                id: id,
                element: element,
        ...
        check: function () {
            ...
            for (var i in modules) {
                ...
                module.visible = ((elementBottom <= viewBottom) && (elementTop >= viewTop));
                if (module.visible) {
                    if (!module.seen || module.repeat) {
                        module.callback(module.element);
                    }
                    module.seen = true;
                }
            }
    ...
    window.HeleoTracking = HeleoTracking;

    $(function () {
        $('.heleo-module').each(function () {
            HeleoTracking.Module.registerModule($(this), function (element) {
                var moduleName = element.data('moduleName');
                HeleoTracking.Event.trackImpression(moduleName);
            }, false);
        });
        HeleoTracking.Module.check();
        $window.scroll(HeleoTracking.Module.check);
    });
...
;(function ($) {
    ...
    Wizard.prototype = {
        ...
        test: function (uri, method, args) {
            ...
            return this[method](uri, args).then(function (data) {
                console.log(data);
            }, function (err) {
                console.error(err);
            });
